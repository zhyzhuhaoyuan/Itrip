...
//			user.setBaidu(userVO.getBaidu());
	...
	Dto registerbyphone(
		...
		if(!validPhone(userVO.getUserCode()))
			return  DtoUtil.returnFail("请使用正确的电话号码",ErrorCode.AUTH_ILLEGAL_USERCODE);
...
//			user.setBaidu(userVO.getBaidu());
	...
	@ApiOperation(value="用户名验证",httpMethod = "GET",
			protocols = "HTTP", produces = "application/json",
			response = Dto.class,notes="验证是否已存在该用户名")
	@RequestMapping(value="/cksms",method=RequestMethod.GET,produces= "application/json")
	public @ResponseBody
	Dto ckSms(
			@ApiParam(name="name",value="被检查的用户名",defaultValue="test@bdqn.cn")
			@RequestParam String name) {
	...
	@ApiOperation(value="电话注册用户激活",httpMethod = "POST",
			protocols = "HTTP", produces = "application/json",
			response = Dto.class,notes="电话激活")
	@RequestMapping(value="/validatephone",method=RequestMethod.PUT,produces= "application/json")
	public @ResponseBody
	Dto validatephone(
			@ApiParam(name="user",value="注册电话",defaultValue="test@bdqn.cn")
			@RequestParam String user,
			@ApiParam(name="code",value="激活码",defaultValue="018f9a8b2381839ee6f40ab2207c0cfe")
			@RequestParam String code){
		try {
